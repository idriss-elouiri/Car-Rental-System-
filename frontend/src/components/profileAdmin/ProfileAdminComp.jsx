...
export default function ProfileAdminComp() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(
        `${apiUrl}/api/adminUser/update/${currentUser._id}`,
        {
          method: "PUT",
          credentials: "include",
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${apiUrl}/api/adminUser/delete/${currentUser._id}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleSignout = async () => {
    try {
      const res = await fetch(`${apiUrl}/api/staff/signoutStaff`, {
        method: "POST",
      });
      const data = await res.json();
      if (!res.ok) {
  ...
  return (
    <motion.div
      className="bg-gray-800 bg-opacity-50 backdrop-blur-md w-[50%] mx-auto min-h-screen shadow-lg rounded-xl p-6 border border-gray-700 mb-8"
    ...
    >
      <div className="max-w-lg mx-auto p-3 w-full">
        ...
        {updateUserError && (
          <div className="mt-5 text-red-600 text-center">{updateUserError}</div>
        )}
